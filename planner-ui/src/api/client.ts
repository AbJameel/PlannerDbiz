...
async function loadConfig(): Promise<AppConfig | null> {
  try { const response = await fetch('/config.json', { cache: 'no-store' }); if (!response.ok) return null; const json = await response.json(); return json as AppConfig; } catch { return null; }
}
...
export const api = {
  ...
  createTask: (payload: { subject: string; fromEmail: string; body: string }) => request('/tasks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }),
